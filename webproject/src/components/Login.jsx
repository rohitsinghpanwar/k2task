...
function Login() {
  ...
  return (
    <>
      <form className='bg-gradient-to-tr from-blue-500 h-[100vh] flex flex-col justify-center items-center' onSubmit={handleSubmit}>
        <div className='flex flex-col bg-white/30 lg:h-[40vh]  rounded-xl lg:w-[50vw] p-5 h-[50vh] justify-around'>
          ...
          <div className='flex justify-between gap-3'>
            <Link to={"/signup"} className='font-semibold underline underline-offset-2 self-center text-sm lg:text-lg'> Create New Account ?</Link>
            <Link to={"/forgotpassword"} className='font-semibold underline underline-offset-2 self-center text-sm lg:text-lg'> Forgot Password ?</Link>
          </div>
          <button type="submit" className='text-xl bg-blue-800 w-24 lg:h-10 rounded-full self-center'>Log-in</button>
          {submit && (
            <>
              <p className='text-center bg-green-400 text-lg italic rounded-lg shadow-xl'>Log-in Successful</p>
            </>
          )}
          {error && (
